...
const Appointments = () => {
  ...
  const { appointments, loading } = useAppointments(user?.organizationId || '');
        ...
        <div className="divide-y divide-slate-100">
          {appointments.length === 0 ? (
            <div className="p-12 text-center">
              ...
              <p className="text-slate-500">Crea una nueva cita para comenzar.</p>
            </div>
          ) : (
              ...
              return (
                <div key={apt.id} className="p-6 flex flex-col lg:flex-row lg:items-center gap-6 hover:bg-slate-50 transition-colors group">
                  ...
                  <div className="flex-1 flex items-center gap-4">
                    {contact ? (
                      <>
                        <img src={contact.avatar || `https://ui-avatars.com/api/?name=${contact.name}`} alt="" className="w-12 h-12 rounded-full bg-slate-200 object-cover border border-slate-200" />
                        <div>
                      ...
                      <div className="flex items-center gap-4 opacity-50">
                        <div className="w-12 h-12 rounded-full bg-slate-100 flex items-center justify-center"><User className="w-6 h-6 text-slate-300" /></div>
                        <p className="text-sm font-bold text-slate-400">Contacto eliminado</p>
                      </div>
                    )}
                  ...
                  <div className="flex items-center gap-8 min-w-[300px]">
                    ...
                    <div className="ml-auto">
                      <span className={`text-xs font-bold px-3 py-1 rounded-full border flex items-center gap-1.5 ${isConfirmed ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
