...
const Contacts = () => {
  ...
  // Data State
  const { contacts, loading: loadingContacts } = useContacts(user?.organizationId || '');

  // TODO: Fetch real agents from DB. For now, we simulate current user is the agent.
  const ALL_AGENTS = user ? [user] : [];

  const loading = loadingAuth || loadingContacts;
  ...
  const handleOpenCreate = () => {
    setFormData({ ...emptyContact }); // ID will be generated
    setIsEditing(false);
    setIsModalOpen(true);
  ...
  const handleDelete = async (id: string) => {
    if (window.confirm('¿Estás seguro de eliminar este contacto?')) {
      try {
        const { error } = await supabase.from('comm_contacts').delete().eq('id', id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...
        avatar: isEditing ? formData.avatar : `https://i.pravatar.cc/150?u=${formData.name.replace(/\s+/g, '')}`,
      ...
      if (isEditing) {
        ...
        const { error } = await supabase
          ...
          .insert({
            ...payload,
            id: `c_${Date.now()}_${Math.random().toString(36).substr(2, 9)}` // Simple ID gen
          });
        if (error) throw error;
      }
  ...
  const processedContacts = useMemo(() => {
    ...
    return data;
  }, [contacts, searchTerm, sortConfig, onlyVip]);
  ...
  return (
    <div className="flex-1 overflow-y-auto p-4 md:p-8 h-full custom-scrollbar relative">
        ...
        <div className="flex flex-col md:flex-row gap-4 mt-2">
          ...
          <div className="flex gap-3 overflow-x-auto pb-1 no-scrollbar">
            <button
              ...
              className={`px-4 py-2 rounded-xl font-bold text-sm flex items-center gap-2 border transition-all whitespace-nowrap ${onlyVip
      ...
      {/* Table View */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden mb-20 min-h-[400px] relative">
        {loading && (
        ...
        <div className="bg-slate-50 px-6 py-4 border-t border-slate-200 flex items-center justify-between">
          <p className="text-xs text-slate-500 font-medium">
            Mostrando <span className="font-bold text-slate-900">{processedContacts.length}</span> de <span className="font-bold text-slate-900">{contacts.length}</span> contactos
          </p>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg relative z-10 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={() => setIsModalOpen(false)} disabled={saving} className="text-slate-400 hover:text-slate-600 transition-colors disabled:opacity-50">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 overflow-y-auto max-h-[70vh] custom-scrollbar space-y-4">
              ...
              <div className="space-y-4">
                    ...
                    >
                      ...
                      {ALL_AGENTS.map(agent => (
                        <option key={agent.id} value={agent.id}>
                          {agent.name} {agent.id === user?.id ? '(Tú)' : ''} - {agent.role}
                        </option>
                      ))}
                    </select>
                ...
                {/* VIP Toggle */}
                <div className="flex items-center gap-3 p-3 bg-amber-50 rounded-xl border border-amber-100 cursor-pointer" onClick={() => !saving && setFormData({ ...formData, isVip: !formData.isVip })}>
                  ...
                  <div className="flex gap-2 mb-2 flex-wrap">
                    {formData.tags.map(tag => (
                      <span key={tag} className="bg-primary-50 text-primary-700 px-2 py-1 rounded-lg text-xs font-bold flex items-center gap-1">
                        #{tag}
                        <button onClick={() => removeTag(tag)} className="hover:text-red-500" disabled={saving}><X className="w-3 h-3" /></button>
                      </span>
                    ))}
                  </div>
                  <div className="flex gap-2">
                    <input
                      ...
                      disabled={saving}
                    />
                    <button onClick={addTag} disabled={saving} className="bg-slate-100 hover:bg-slate-200 text-slate-600 px-3 py-2 rounded-lg font-bold text-xs">
            ...
            <div className="bg-slate-50 p-6 border-t border-slate-100 flex justify-end gap-3">
              <button
                ...
                disabled={saving}
                className="px-5 py-2.5 text-slate-500 font-bold text-sm hover:bg-slate-200 rounded-xl transition-colors disabled:opacity-50"
              >
                Cancelar
              </button>
              <button
                onClick={handleSave}
                disabled={saving}
                className="px-6 py-2.5 bg-primary-600 text-white font-bold text-sm rounded-xl hover:bg-primary-700 shadow-lg shadow-primary-200 transition-colors flex items-center gap-2 disabled:opacity-50"
              >
                {saving ? (
                ...
                {isEditing ? (saving ? 'Guardando...' : 'Guardar Cambios') : (saving ? 'Creando...' : 'Crear Contacto')}
              </button>
            </div>
          </div>
        </div>
