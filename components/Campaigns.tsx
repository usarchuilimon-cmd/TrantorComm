...
import { CAMPAIGNS, CONTACTS, WHATSAPP_TEMPLATES } from '../mockData';
...
const Campaigns = () => {
  ...
  const selectedTemplate = WHATSAPP_TEMPLATES.find(t => t.id === newCampaign.templateId);
  ...
  const handleCreate = () => {
    ...
    const campaign: Campaign = {
      ...
      sentDate: newCampaign.scheduled ? newCampaign.scheduledDate : 'Ahora',
      audienceSize: estimatedAudience,
      deliveredCount: 0,
      readCount: 0,
      templateName: selectedTemplate?.name || 'Desconocido',
      tagFilter: newCampaign.tagFilter
    };

    setActiveCampaigns([campaign, ...activeCampaigns]);
    
    // Simulate processing if running immediately
    if (!newCampaign.scheduled) {
      alert(`🚀 Enviando campaña a ${estimatedAudience} contactos...\n\nEsta acción consumirá créditos de Meta.`);
    }
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 animate-in fade-in slide-in-from-right-4 duration-300">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
                    {WHATSAPP_TEMPLATES.filter(t => t.status === 'APPROVED').map(template => (
                      <div 
                        key={template.id}
      ...
      <div className="max-w-[1200px] mx-auto animate-in fade-in duration-500">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
           <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex items-center gap-4">
              ...
              <div>
                <p className="text-2xl font-extrabold text-slate-900">1,548</p>
                       ...
                       {camp.status !== 'scheduled' ? (
                          <div className="space-y-1">
                            <div className="flex justify-between text-[10px] font-bold text-slate-500">
                              <span>Entregados: {Math.round((camp.deliveredCount / camp.audienceSize) * 100)}%</span>
                              <span>Leídos: {Math.round((camp.readCount / camp.audienceSize) * 100)}%</span>
                            </div>
